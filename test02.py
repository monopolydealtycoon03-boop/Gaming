...
<head>
    ...
    <style>
        ...
        .game-card.disabled-card { opacity: 0.7; } 
        .game-card.discard-mode { border: 2px solid #ef4444; animation: shake 0.5s infinite; }
    ...
    <div id="game-screen" class="hidden p-2 md:p-4 max-w-[1600px] mx-auto w-full flex-col justify-between space-y-4 min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 flex-1">
            <div class="lg:col-span-3 space-y-4 flex flex-col justify-between">
                ...
                <!-- PLAYER BOARD AREA -->
                <div id="my-board-container" class="bg-black/50 border border-gold-500/40 rounded-xl p-4 flex-1 flex flex-col justify-between min-h-[260px] transition-all">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 flex-1">
                        <!-- Player Bank -->
                        <div class="md:col-span-1 bg-cardbg/80 border border-emerald-500/30 rounded-lg p-2.5 flex flex-col">
                        ...
                        <!-- Player Properties -->
                        <div class="md:col-span-3 bg-cardbg/80 border border-gold-500/30 rounded-lg p-2.5 flex flex-col">
                            <div class="flex justify-between items-center mb-2">
                ...
                <div class="bg-cardbg border-2 border-gold-500/60 rounded-xl p-3 shadow-xl">
                    <div class="flex justify-between items-center mb-2 px-1">
                        ...
                        <div class="text-[10px] text-slate-400">Click a card to choose action</div>
                    </div>
            ...
            <!-- Turn Log & Stats -->
            <div class="bg-cardbg border border-gold-500/30 rounded-xl p-4 flex flex-col justify-between h-full space-y-4">
                <div class="flex-1 flex flex-col min-h-[300px]">
                    <div class="text-xs font-bold text-gold-500 uppercase tracking-widest mb-2 border-b border-gold-500/20 pb-1 flex items-center gap-1.5">
                        <i class="fa-solid fa-scroll"></i>
                        <span>Live Action Log</span>
                    </div>
                    <div id="game-log" class="flex-1 bg-black/80 border border-slate-800 rounded-lg p-2.5 overflow-y-auto font-mono text-[11px] space-y-1.5 max-h-[500px]"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        socket.on('alert', (data) => {
            ...
            if (msg.includes('just say no') || msg.includes('deal breaker') || msg.includes('stole') || msg.includes('snatched') || msg.includes('shattered')) {
                playSound('alert');
            } else if (msg.includes('rent') || msg.includes('paid') || msg.includes('debt') || msg.includes('birthday') || msg.includes('bank')) {
                playSound('cash');
            } else {
                playSound('deal');
            }
        ...
        socket.on('game_over', (data) => {
            ...
            let winHtml = `
                <div class="text-center space-y-4 py-4">
                    ...
                    <h2 class="font-cinzel text-2xl font-extrabold text-gold-300">${data.winner} Wins!</h2>
                    <button onclick="location.reload()" class="modal-btn mt-4">Exit to Lobby</button>
                </div>
            `;
...
# ==========================================
# 2. PYTHON BACKEND (SAME PRO AI ENGINE)
# ==========================================
SET_SIZES = {
