...
# ==========================================
# 1. HTML & JAVASCRIPT FRONTEND (WITH ANIMATED BACKGROUND)
# ==========================================
HTML_PAGE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        body {
            ...
            text-align: center;
            background: linear-gradient(-45deg, #1a1c29, #2c3e50, #1b263b, #0f172a);
        ...
        #done-turn-btn:hover { background-color: #219653; }
        ...
        input, button { padding: 10px; margin: 5px; font-size: 16px; border-radius: 6px; border: none; width: 90%; max-width: 300px; outline: none; }
        ...
        .turn-box { background: #e67e22; padding: 12px; border-radius: 8px; margin-bottom: 15px; font-size: 16px; font-weight: bold; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
        .disabled-card { opacity: 0.5; cursor: not-allowed; }
        .discard-mode { border: 3px solid #e74c3c !important; box-shadow: 0 0 15px #e74c3c; animation: pulse 1s infinite; }
        @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }
        
        .sets-container { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; margin-top: 5px; }
        .set-column { border: 1px dashed rgba(255,255,255,0.4); padding: 6px; border-radius: 6px; background: rgba(0,0,0,0.2); min-width: 100px; text-align: center; }
        
        #modal-overlay {
            display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(0,0,0,0.8); z-index: 2500; justify-content: center; align-items: center;
            backdrop-filter: blur(5px);
        }
        .modal-content { background: #34495e; padding: 20px; border-radius: 10px; width: 90%; max-width: 450px; text-align: center; border: 2px solid white; max-height: 80vh; overflow-y: auto; }
        .modal-btn { background: #2980b9; margin: 6px 0; width: 100%; cursor: pointer; padding: 12px; color: white; font-weight: bold; border-radius: 5px; border: none; }
        .modal-btn:hover { background: #3498db; }
        .target-prop-option { padding: 10px; margin: 6px 0; background: #2c3e50; border: 1px solid white; border-radius: 6px; cursor: pointer; color: white; }
        .target-prop-option:hover { background: #e67e22; }
    </style>
</head>
<body>
    ...
    <div id="modal-overlay">
        <div class="modal-content">
            ...
            <button onclick="closeModal()" style="background: #c0392b; margin-top: 15px; cursor:pointer; color: white; padding: 10px; border-radius: 5px; border:none;">Cancel</button>
        </div>
    </div>
    ...
    <div id="lobby-screen" class="screen active">
        ...
        <input type="text" id="player-name" placeholder="Enter Your Name" required><br>
        
        <div class="box">
            <h3>Host a Game</h3>
            <input type="text" id="create-room-id" placeholder="Create Room ID"><br>
            <input type="password" id="create-room-pass" placeholder="Create Password"><br>
            <button onclick="goToModeSelect()" style="background-color: #3498db; cursor:pointer; color:white; font-weight:bold;">Create Room</button>
        </div><br>

        <div class="box">
            <h3>Join a Friend</h3>
            <input type="text" id="join-room-id" placeholder="Friend's Room ID"><br>
            <input type="password" id="join-room-pass" placeholder="Password"><br>
            <button onclick="joinRoom()" style="background-color: #e67e22; cursor:pointer; color:white; font-weight:bold;">Join Room</button>
        </div>
    </div>
    ...
    <div id="mode-select-screen" class="screen">
        ...
        <div class="box">
            ...
            <p style="font-size: 14px; opacity: 0.8; margin-bottom: 5px;">Select Difficulty Level:</p>
            <button onclick="createRoomWithBot('easy')" style="background-color: #27ae60; cursor:pointer; color:white; font-weight:bold;">🟢 Easy Bot</button>
            <button onclick="createRoomWithBot('normal')" style="background-color: #f39c12; cursor:pointer; color:white; font-weight:bold;">🟡 Normal Bot</button>
            <button onclick="createRoomWithBot('hard')" style="background-color: #c0392b; cursor:pointer; color:white; font-weight:bold;">🔥 Hard Bot (EXTREME BOSS)</button>
        </div><br>
        <div class="box">
            <h3>👥 Play with Friends</h3>
            <p style="font-size: 14px; opacity: 0.8;">Host an online room for your friends to join via Room ID.</p>
            <button onclick="createRoomWithMode('multiplayer')" style="background-color: #2980b9; cursor:pointer; color:white; font-weight:bold;">Play Multiplayer</button>
        </div><br>
        <button onclick="backToLobby()" style="background-color: #7f8c8d; width: 120px; cursor:pointer; color:white; font-weight:bold;">Back</button>
    </div>

    <!-- WAITING ROOM -->
    <div id="waiting-screen" class="screen">
        <h2>Room ID: <span id="display-room-id" style="color:#f1c40f;"></span></h2>
        <h3>Players in Lobby:</h3>
        <ul id="player-list"></ul>
        <div id="host-controls" style="display: none; margin-top: 20px;">
            <button onclick="startGame()" style="background: #27ae60; padding: 12px 25px; font-size: 18px; cursor:pointer; color:white; font-weight:bold;">▶ Start Game</button>
        </div>
    </div>
    ...
    <div id="game-screen" class="screen">
        ...
        <div style="background: rgba(0,0,0,0.3); padding: 12px; border-radius: 10px; margin-bottom: 15px; backdrop-filter: blur(5px);">
            <h3>Opponents Table</h3>
            <div id="opponents-area"></div>
        </div>

        <div style="background: rgba(255,255,255,0.1); padding: 12px; border-radius: 10px; backdrop-filter: blur(5px);">
            <h3>Your Hand</h3>
            <div id="my-hand"></div>
        </div>
    </div>
    ...
    <script>
        ...
        socket.on('update_state', (data) => {
            ...
            if (data.started && !data.game_over) {
                ...
                if (myTurn) {
                    document.getElementById('turn-info').style.background = isDiscardMode ? "#c0392b" : "#27ae60";
                    document.getElementById('done-turn-btn').style.display = isDiscardMode ? 'none' : 'block';
                } else {
                    document.getElementById('turn-info').style.background = "#e67e22";
                    document.getElementById('done-turn-btn').style.display = 'none';
                }
            ...
            data.players.forEach(p => {
                ...
                let propHtml = sortedColors.map(col => {
                    ...
                    let completeBadge = isComplete ? ' <br><span style="color: #2ecc71; font-weight: bold; font-size: 10px;">(COMPLETE!)</span>' : '';

                    return `<div class="set-column">
                                <div style="font-size: 12px; font-weight: bold; margin-bottom: 4px; color: #f1c40f;">${col}${completeBadge}</div>
                                ${cardsList} ${bldgs}
                            </div>`;
                }).join('');
                
                let activeStyle = p.is_turn ? "border: 2px solid #f1c40f; background: rgba(241, 196, 15, 0.2);" : "border-bottom: 1px solid rgba(255,255,255,0.2);";
                
                oppHtml += `
                    <div style="padding: 8px; margin-bottom: 8px; border-radius: 8px; ${activeStyle}">
                        <strong>${p.name}</strong> (Cards: ${p.card_count})<br>
...
# ==========================================
# 2. PYTHON BACKEND & STRICT BOT RENT VALIDATION
# ==========================================
SET_SIZES = {
    'Brown': 2, 'Light Blue': 3, 'Pink': 3, 'Orange': 3,
    'Red': 3, 'Yellow': 3, 'Green': 3, 'Dark Blue': 2,
    ...
    def run_bot_turn(self):
        ...
        if self.bot_difficulty == 'easy':
            ...
            while self.actions_left > 0 and bot["hand"]:
                ...
                if db_idx != -1 and human_player:
                    ...
                    if complete_col:
                        ...
                        def execute_extreme_db():
                            to_steal = [p for p in human_player["properties"] if (p.get('color') or 'Wild') == complete_col]
                            for p in to_steal:
                ...
                if not action_played:
                    bot_colors = [p.get('color') for p in bot["properties"] if p.get('color')]
                    rent_idx = -1
                    matched_col = None
                    ...
                    if rent_idx != -1 and matched_col:
                        ...
                        count = sum(1 for p in bot["properties"] if p.get('color') == matched_col)
...
def check_and_demolish_buildings(player):
    for color, bldgs in list(player["buildings"].items()):
        if not bldgs:
            continue
        current_count = sum(1 for p in player["properties"] if p.get("color") == color)
                ...
                if 0 <= card_index < len(player["hand"]):
                    ...
                    if use_type == 'bank':
                        ...
                        if played_card['name'] == 'Property Wild Card' or "&" in played_card['name']:
                            assigned_color = extra_data.get('assigned_color', 'Wild')
                        ...
                        color_group = extra_data.get('color_group', '')
                        current_props = [p for p in player["properties"] if p.get("color") == color_group]
                        ...
                        if played_card['name'] == "Pass Go":
                            ...
                            if target_sid:
                                def execute_sly():
                                    ...
                                    if 0 <= target_prop_idx < len(target_props):
                                        ...
                                        t_col = target_prop.get('color') or 'Wild'
                            ...
                            if target_sid:
                                def execute_forced():
                                    ...
                                    if 0 <= my_prop_idx < len(my_props) and 0 <= target_prop_idx < len(target_props):
                                        ...
                                        my_col = my_card.get('color') or 'Wild'
                                        their_col = their_card.get('color') or 'Wild'
                            ...
                            target_name = extra_data.get('target_name')
                            color_group = extra_data.get('color_group', '').title()
                            target_sid = next((tsid for tsid, pdata in room.players.items() if pdata['name'].lower() == str(target_name).lower() and tsid != sid), None)
                            
                            if target_sid:
                                def execute_db():
                                    target_player = room.players[target_sid]
                                    target_props_in_color = [p for p in target_player["properties"] if p.get("color") == color_group]
                            ...
                            rent_color = extra_data.get('rent_color', '').title()
                            target_name = extra_data.get('target_name', '').strip()
                            ...
                            owned_count = sum(1 for p in player["properties"] if (p.get("color") or "").title() == rent_color)
                            if owned_count > 0:
