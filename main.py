...
<body class="bg-darkbg min-h-screen text-slate-100 flex flex-col justify-between">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                <div class="text-left space-y-4">
                    ...
                    <h1 class="font-cinzel text-4xl md:text-5xl font-extrabold text-gold-300 leading-tight">
                        MONOPOLY <span class="block text-gold-500 text-5xl md:text-6xl gold-text-glow">DEAL</span>
                    </h1>
                    
                    <div class="pt-4 space-y-3">
                        <label class="block text-xs uppercase text-gold-500 font-bold tracking-wider">Your Nickname</label>
                        <input type="text" id="player-name" placeholder="Monopoly Baron" required class="w-full bg-black/60 border border-gold-500/50 rounded-lg p-3 text-white focus:outline-none focus:border-gold-500 font-semibold">
                    </div>
    ...
    <div id="game-screen" class="hidden p-2 md:p-4 max-w-[1600px] mx-auto w-full flex-col justify-between space-y-4 min-h-screen">
        ...
        <div class="bg-cardbg border border-gold-500/40 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3 shadow-lg">
            <div class="flex items-center gap-3">
                <span class="font-cinzel text-gold-500 font-extrabold text-lg tracking-wider">MONOPOLY ARENA</span>
    ...
    <script>
        ...
        // =====================================
        // DRAG AND DROP MECHANICS (DESKTOP)
        // =====================================
        ...
        // =====================================
        // TOUCH EVENTS MECHANICS (MOBILE SUPPORT)
        // =====================================
        ...
        function handleTouchStart(e, idx) {
            if (!myTurn || isDiscardMode || actionsLeft <= 0) return;
            ...
            if (isTouchDragging) {
                ...
                if (elUnderFinger) {
                    if (elUnderFinger.id === 'bank-dropzone' || bankDz.contains(elUnderFinger)) {
                        bankDz.classList.add('bg-emerald-900/80');
                    } else if (elUnderFinger.id === 'property-dropzone' || propDz.contains(elUnderFinger)) {
                        propDz.classList.add('bg-gold-900/80');
                    }
                }
            }
        }
        ...
        function handleTouchEnd(e) {
            ...
            if (elUnderFinger) {
                if (elUnderFinger.id === 'bank-dropzone' || bankDz.contains(elUnderFinger)) {
                    ...
                    handleDrop({ preventDefault: () => {} }, 'property');
                } else {
                    draggedCardIndex = -1;
                }
            } else {
        ...
        function handleDrop(e, type) {
            ...
            if (type === 'bank') {
                if (card.type === 'Money') {
                    ...
                    showToast("Cannot Bank", "Only Cash cards can be deposited to the Bank!", true);
                }
        ...
        function createCardDOM(card, index, clickable, delay = 0) {
            ...
            if (clickable) {
                ...
                if (!isDiscardMode) {
                    ...
                    cardEl.addEventListener('touchcancel', handleTouchEnd);
                }
            }
                ...
                if 0 <= card_index < len(player["hand"]):
                    ...
                    if use_type == 'bank': 
                        if played_card['type'] == 'Money':
                            ...
                            return emit('alert', {'message': 'Only Money cards can be placed in the bank!'}, room=sid)
